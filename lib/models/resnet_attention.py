...
class Attention_net(nn.Module):
    def __init__(self, size=[512, 256], weight_num=1, num_classes=2):
        super(Attention_net, self).__init__()
        self.atten_a = nn.Sequential(
            nn.Linear(size[0], size[1]),
            nn.Tanh(),
            nn.Dropout(0.25)
        )
        self.atten_b = nn.Sequential(
            nn.Linear(size[0], size[1]),
            nn.Sigmoid(),
            nn.Dropout(0.25)
        )
        self.atten_c = nn.Linear(size[1], weight_num)
        self.classifier = nn.Linear(size[0], num_classes)

    def forward(self, x):
        a = self.atten_a(x)
        b = self.atten_b(x)
        tmp = torch.mul(a, b)
        A = self.atten_c(tmp) # 权重
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.attention_net = Attention_net(size=[512, 256], num_classes=num_classes)
    ...
    def _forward_impl(self, x, patch_number):
        ...
        x = x.view(
            int(b/patch_number), patch_number, -1
        )
        # x = self.conv1d_my(x)
        # x = x.squeeze()
    ...
    def forward(self, x, patch_number):
        return self._forward_impl(x, patch_number)
...
if __name__ == '__main__':
    kwargs = {"num_classes":2, "patch_number":64}
    model = resnet18(pretrained=False,**kwargs)
    data = torch.rand((640, 1, 256, 32))
    y = model(data)
    print(y.shape)